    ...
    <div class="overflow-hidden">
        ...
        <main class="py-14 md:py-20 bg-gray-100">
            <div class="mx-4 md:mx-10 lg:mx-16 flex flex-wrap lg:flex-nowrap lg:gap-4">
                <div class="w-full">
                    <div class="flex mb-2">
                        <div class="truncate text-ellipsis w-32 md:w-40 rounded-full bg-emerald-500 text-white font-bold px-4 py-2 text-sm">{{categoryName}}</div>
                    </div>
                    <div class="mb-8">
                    ...
                    <template v-if="!sloading">
                        <div v-for="(post, index) in categoryPost" :key="index" class="flex bg-white rounded overflow-hidden shadow-sm h-36 xl:h-44">
                        ...
                        <template v-if="categoryPost.length == 0">
...
export default {
    ...
    setup() {
        ...
        const getCategoryPosts = () => {
            sloading.value = true
            api.get(`/api/lists/posts/postscategory/${slug.value}?month=${currMonth.value+1}&year=${currYear.value}`).then(res => {
                categoryPost.value = res.data.data
        ...
        const loadmore = () => {
            ...
            if (currLinks.value) {
                api.get(`${currLinks.value}&month=${currMonth.value+1}&year=${currYear.value}`).then(res => {
                    categoryPost.value.push(...res.data.data)
                    currLinks.value = res.data.links.next
                    loading.value = false
                })
            }
        ...
        onMounted(() => {
            categoryName.value = slug.value.replace(/[-]/g, ' ')
        ...
        return { categoryName, categoryPost, slug, categories, tags, currLinks, loadmore, loading, sloading, monthF, yearF, monthYears, currMonth, currYear, changeM, changeY }
    },
}
</script>