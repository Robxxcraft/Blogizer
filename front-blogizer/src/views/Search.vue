            ...
            <div class="mx-4 md:mx-10 lg:mx-16 flex flex-wrap lg:flex-nowrap lg:gap-4">
                <div class="w-full">
                    <div class="inline-block flex mb-2">
                        <div class="truncate text-ellipsis w-30 md:w-36 rounded-full bg-emerald-500 text-white font-bold px-4 py-2 text-sm">{{searched}}</div>
                    </div>
                    <div class="flex justify-between mb-4">
                        <div>
                            <div class="flex font-bold text-base md:text-xl text-gray-700 mb-2 tracking-wide">
                                <div class="mr-4">Search Posts</div>
                            </div>
                        ...
                        <template v-if="!sloading">
                            <div v-for="(post, index) in searches" :key="index" class="flex bg-white rounded overflow-hidden shadow-sm h-36 xl:h-44">
...
export default {
    ...
    setup() {
        ...
        const getSearchs = () => {
            sloading.value = true
            api.get(`/api/lists/posts/search/posts?search=${searched.value}&month=${currMonth.value+1}&year=${currYear.value}`).then(res => {
        ...
        const loadmore = () => {
            ...
            if (currLinks.value) {
                api.get(`${currLinks.value}&month=${currMonth.value+1}&year=${currYear.value}`).then(res => {
                    searches.value.push(...res.data.data)
                    currLinks.value = res.data.links.next
                    loading.value = false
                })
            }
        ...
        return { searched, searches, categories, tags, currLinks, loadmore, loading, sloading, monthF, yearF, monthYears, currMonth, currYear, changeM, changeY}
    },
}
</script>