<template>
  <header class="flex items-center px-4 justify-between">
    <div class="py-4 md:px-4 text-lg text-emerald-500 font-bold">Create</div>
  ...
  <section class="p-4 md:p-8 md:mt-0">
      ...
      <div class="text-xl text-gray-700 font-bold mt-4 mb-2">Create Category</div>
      <form class="w-full mx-auto md:px-8 py-4" @submit.prevent="saveCategory">
          <div class="flex max-w-lg mx-auto flex-wrap px-0 md:px-8  flex-col">
              ...
              <div class="flex-1 text-right">
                <button class="text-white tracking-wide bg-emerald-500 py-2 px-3 rounded text-sm font-bold shadow-sm transition focus:outline-none hover:bg-green-600 hover:shadow-lg" type="submit">Save</button>
              </div>
          </div>
      </form>
  </section>
  ...
  setup() {
    ...
    const saveCategory = () => {
      api.post('/api/categories', form).then(()=>{
        router.push({name: 'ManageCategory', params: {alert_title: 'Success', alert_subtitle: 'Category has been created.'}});
      }).catch(err => {
        errors.value = err.response.data.errors
      })
    }

    return { side, saveCategory, errors, form }
  }
};
</script>
