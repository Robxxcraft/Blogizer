<template>
    ...
    <div class="py-4 block md:hidden text-emerald-500 hover:text-green-600" id="header" @click="side">
      ...
      <section class="p-4 md:p-8 md:mt-0">
        ...
        <div class="text-xl text-gray-700 font-bold mb-2">Manage Tags</div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:hidden gap-4">
          <div class="bg-white p-4 rounded-lg shadow-sm" v-for="(tag, index) in tags" :key="index">
            ...
            <div class="ml-8">
              <div class="text-sm truncate two-lines text-gray-700 mb-1 font-bold">{{tag.name}}</div>
              <div class="text-gray-500 text-xs">{{tag.slug}}</div>
            </div>
          </div>
        </div>
...
export default {
  ...
  setup(props) {
    ...
    const getTags = (page) => {
      sloading.value = true
      api.get(`/api/tags?page=${page}&search=${search.value}`).then(res => {
    ...
    return { side, currentPage, pagination, search, searching, nextLink, sloading, tags, getTags, deleteTag }
  }
}
</script>
