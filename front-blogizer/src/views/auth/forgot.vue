<template>
    ...
    <main class="bg-white max-w-lg mx-auto px-8 py-auto md:rounded md:shadow-2xl h-screen md:h-full relative">
        <div v-if="alert" class="absolute inset-x-0 w-full text-sm bg-green-100 border-b border-emerald-500 text-green-800 px-3 py-1.5 rounded-md shadow-sm" role="alert">
        ...
        <section class="mt-24">
            <form @submit.prevent="sendemail">
                ...
                <div class="flex flex-col items-end mt-28">
                    <button class="bg-emerald-500 hover:bg-green-600 text-white font-bold px-3 py-2 focus:outline-none rounded shadow-lg hover:shadow-xl transition duration-200" type="submit">Send</button>
                </div>
            </form>
        </section>
